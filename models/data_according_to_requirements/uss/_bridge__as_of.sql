...
WITH cte__union AS (
  SELECT
    ...
    record_valid_to,
    is_current_record
  FROM data_according_to_requirements.uss__staging._bridge__categories
  UNION ALL
  SELECT
    peripheral,
    NULL AS _pit_hook__product__id,
    ...
    record_valid_to,
    is_current_record
  FROM data_according_to_requirements.uss__staging._bridge__category_details
  UNION ALL
  SELECT
    peripheral,
    ...
    record_valid_to,
    is_current_record
  FROM data_according_to_requirements.uss__staging._bridge__products
)
