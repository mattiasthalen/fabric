...
WITH cte__record_validity AS (
  SELECT
    @STAR(
      relation := das.scd.scd__northwind__employees,
    ...
    ROW_NUMBER() OVER (PARTITION BY employee_id ORDER BY _record__valid_from ASC) AS _record__version
...
SELECT
  ...
  _hook__region__id,
  @STAR__LIST(
    table_name := das.scd.scd__northwind__employees,
    exclude := [_record__loaded_at, _record__valid_from, _record__valid_to],
  ),
  _record__loaded_at,
  _record__updated_at,
